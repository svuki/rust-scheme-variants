...
fn eval(env: Rc<RefCell<Env>>, expr: ValueRef) -> ValueRef {
    ...
    match &*expr.borrow() {
        ...
        Value::Cons(car, cdr) if expect_symbol(&*car.borrow()).map(is_special) == Some(true) => {
        ...
        Value::PrimitiveProc(_) => panic!("Tried to evaluate a primitive proc."),
        Value::Macro(_) => panic!("Tried to evaluated a macro."),
    }
}
...
const SPECIAL_MACRO: &str = "macro";
const SPECIAL_QUOTE: &str = "quote";
const SPECIAL_QUASI_QUOTE: &str = "quasi-quote";
const SPECIAL_UNQUOTE: &str = "unquote";

const SPECIAL_SYMBOLS: [&str; 13] = [
    SPECIAL_CAR,
    SPECIAL_CDR,
    SPECIAL_CONS,
    SPECIAL_IF,
        ...
        match s.as_ref() {
            ...
            SPECIAL_LAMBDA => {
                ...
                let arglist: Vec<String> = args
                    .iter()
                    .map(|x| expect_symbol(&*x.borrow()).unwrap().to_string())
                    .collect();
            ...
            SPECIAL_QUOTE => c_iter.next().unwrap(),
            SPECIAL_UNQUOTE => eval(env.clone(), c_iter.next().unwrap()),
            SPECIAL_QUASI_QUOTE => quasi_eval(env.clone(), c_iter.next().unwrap()),
...
fn quasi_eval(env: Rc<RefCell<Env>>, value: ValueRef) -> ValueRef {
    match &*value.borrow() {
        Value::Cons(car, _) if is_unquote(car.clone()) => {
            let r = apply_special(env, value.clone());
            r
        }
        Value::Cons(car, cdr) => Value::Cons(
            quasi_eval(env.clone(), car.clone()),
            quasi_eval(env.clone(), cdr.clone()),
        )
        .to_ref(),
        _ => value.clone(),
    }
...
fn is_unquote(value: ValueRef) -> bool {
    match &*value.borrow() {
        Value::Symbol(s) => s == SPECIAL_UNQUOTE,
        _ => false,
    }
}

fn is_special_symbol(value: ValueRef) -> bool {
    match &*value.borrow() {
        Value::Symbol(s) => is_special(s),
        _ => false,
    }
}
...
fn apply(env: Rc<RefCell<Env>>, values: ValueRef) -> ValueRef {
    ...
    match &*eval(env.clone(), values[0].clone()).borrow() {
        ...
        Value::Macro(mac) => {
            let args: Vec<ValueRef> = values[1..].to_vec();
            let macro_env = new_child_env(mac.env.clone());
            macro_env.borrow_mut().bind_many(mac.arglist.clone(), args);
            println!("Expanding macro {:#?}", mac.body);
            let expanded = eval(macro_env.clone(), mac.body.clone());
...
fn expect_symbol(v: &Value) -> Option<&str> {
    println!("value: {:#?}", v);
...
";
    ...
    fn test_macro_1() {
        let program = "
(progn
    (define unless (macro (condition if_true_expr)
      (quasi-quote (if (unquote condition) (unquote if_true_expr) 'nil))))
    (unless (= 0 0) 100))
";
        let expr = parse(read(program.to_string()));
        let root_env = Rc::new(RefCell::new(Env::new_with_primitives()));
...
";
        ...
        let root_env = Rc::new(RefCell::new(Env::new_with_primitives()));
        let result = eval(root_env, expr);
        assert_eq!(result, Value::Nil.to_ref());
    }
}
